...
class Ui_Signup(object):
    ...
    def setupUi(self, Dialog):
        ...
        self.remind_passwd.setGeometry(QtCore.QRect(151, 199, 301, 41))
        ...
        self.tip_phone.setGeometry(QtCore.QRect(151, 274, 301, 31))
        ...
        self.return_home.clicked.connect(self.back_signin)
        self.signup.clicked.connect(self.go_main)
        ...
        self.name.setPlaceholderText("字母数字,且不为纯数字.长度在三到八之间")
        self.passwd.setPlaceholderText("密码6-9位，数字字母，必须以字母开头")
    ...
    def nameCheck(self, text):
        if not text:
            self.tip_name.setText("您输入的用户名为空， 请重新输入！")
        elif len(text) < 3:
            self.tip_name.setText("您输入的用户名长度小于3， 请重新输入！")
        else:
            self.tip_name.setText('')

    # 用户首次输入密码判断
    def onChanged(self, text):
        if not text:
            self.tip_name.setText("您输入的密码为空， 请重新输入！")
        elif len(text) < 6:
            self.tip_name.setText("您输入的密码长度小于6， 请重新输入！")
        else:
            self.tip_name.setText('')
    ...
    def phoneNumberCheck(self, text):
        if not text:
            self.tip_name.setText("您输入的内容为空， 请重新输入！")
        elif len(text) < 11:
            self.tip_name.setText('您输入的号码有误， 请重新输入！')
        else:
            self.tip_name.setText('')
    ...
    def mailCheck(self, text):
        if not text:
            self.tip_name.setText("您输入的邮箱为空， 请重新输入！")
        else:
            self.tip_name.setText('')
    ...
    def signup_info(self):
        ...
        if self.tip_name.text() or self.remind_passwd.text():
            self.tip_name.setText('您输入的内容有误, 请检查无误后提交!')
        elif self.passwd_t != self.passwd_confirm_t:
            self.tip_name.setText("您两次输入的密码不一致，请重新输入！")
        elif usr_mail != self.mail_t:
            self.tip_name.setText('您输入的邮箱有误， 请重新输入！')
        else:
            info = self.server.do_register(self.name_t, self.passwd_t,
                                           self.phoneNumber_t, self.mail_t)
            if info[0] == '0000':
                self.go_main()
            elif info[0] == '0001':
                self.tip_name.setText('用户名已存在,请重试')
            else:
                self.tip_name.setText('未知错误,请重试')
